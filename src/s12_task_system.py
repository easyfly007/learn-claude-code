...
Ttasks persiste as JSON files in .tasks so they survive context compression
each task carries a small dependency graph:

    - blockedBy: what must finish first
    - blocks: what this task unblocks later
    ...
    task1           task2       task3
    complted   ==>  blocked ==> blocked
        |
        +-------complteing task1 remove it from task2's blockedBy

key idea:task state survivies compression because it lives on disk, not only 
inside the conversiation
There are durable work-graph tasks, not ttransient runtime execution slots

Read this file in this order:
    1. TaskManager: that a TaskRecord looks like on disk
...
teaching boundary:
    this chapter teachesthe durable work graph first
...
class TaskManager:
    """Persisten TaskRecord store.
    Think "work graph on disk", not "currently worker"
    """
    ...
    def update(self, task_id: int, status: str=None, owner:str=None,
        ...
        if add_blocks:
            ...
            # bidirectional also update the blocked tasks blockedBy list
