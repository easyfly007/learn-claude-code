"""
s14 cron scheduler -- independentdaemon thread + queue processor
...
change from s13:
    ...
    - cron_match: 5-field cron expression matching with DOM/DOW OR semantics
    - schedule_job / cancel_job: register / remove cron jobs (with validation)
    - cron_scheulder_loop: independent daemon thread, polling every 1s
    - cron_queue: thread-safe queue, scheduler writes, queue processor delivers
    - queue_processor_loop: auto-run agent_loop when cron_queue has work
    - Durable storage: .scheduled_task.josn (survives restart)
    - 3 new tools: schedule_cron, list_crones, cancel_crone

four layers:
    1. scheduler: daemon thread checks ttime -> fires matching jobs
    ...
    4. comsumer: agent_loop soncumes queued jobs and injects them into messages
"""
...
_last_fired: dict[str, str] = {} # jobid ->"YYYY-MM-DD HH:MM"
...
def validate_cron(cron_expr: str)->str | None:
    """
    validate a cron expression. returns error mesage or None
    """
    fields = cron_expr.strip().split()
    if len(fields) != 5:
        return f"Expected 5 fields, got {len(fields)}"
    bounds = [(0,59), (0, 23), (1,31), (1,12), (0, 6)]
    names = ["minutes", "hour", "day-of-month", "month", "day-of-week"]
...
def save_durable_jobs():
    """
    persist durable jobs to .scheduled_tasks.josn
    """
