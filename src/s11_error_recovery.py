...
teaching demo of theree recovery paths

- continue when output is truncated
- compact when context grows too large
- back off when transport errors a temporary
    ...
    [check stop_reason]
        |
        +-- "max tokens" -->  [ strategy 1: max_output_tokens recovery]
        ...
        |                            exponetial backoff: base * 2^attempt + jitter
        |                            up to 3 rejtries
        |   
...
4. all tries exhausted -> fail gracefully
"""
...
def auto_compact(messages:list)-> list:
    ...
    continuation = (
            "this session continue from a previous conversation that was compacted."
            f"summary of preior context:\n\n{summary}\n\n"
            "continue from where we left off whout re-asking the user"
            )
    ...
    while True:
        ...
        for attempt in range(MAX_RECOVERY_ATTEMPTS + 1):
            try:
                ...
                # strategy 3: connection / rate error ->backoff
                ...
                print(f"[Error] API call failed after {MAX_RECOVERY_ATTEMPTS} reties: {e}")
                return
