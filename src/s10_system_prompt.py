# harness: assembly -- thesystem prompt is a pipeline, not a string
"""
s10_system_prompt.py - System Prompt construction
this chapter teaches one core idea:
    the system prompt should be assembled from clear section, not written as one
    giatn hardcoded blob
...
the builder keeps stable information separate from information tha changes 
often. a simple dynamic_boundary marker makes that split visible

per-turn reminders are even more dynamic.
theya are bettern injected as a separate user-role system reminder that mixed blindly into the stable prompt
...
class SystemPromptBuilder:
    ...
    each secion has one source and one responsibility

    that makes the prompt easier to reason about, easier to test, and easier to 
    evolve as the agent grows now capabilities.
    """
    ...
    def _build_claude_md(self)->str:
        """
        load claude.md files in preority order (all are included)
        ...
        # subdirectoriy -- in real cc, this walks from cwd up to project root
    ...
    def build(self)->str:
        ...
        static section (1-5) are separated from dynamic (6) by
...
def build_system_reminder(extra: str = None) -> str:
    """
    build a system -reminder user message for per-turn dynami content
    the teaching version keeps reminders outside the stable system prompt so 
    short-live context does not get mixed into the long-lived instructions.
    """
    parts = []
    if extra:
        parts.append(extra)
